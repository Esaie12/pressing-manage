...
<div class="row g-3">
  <div class="col-lg-3">
    <div class="list-group shadow-sm">
      <a href="{{ route('employee.ui.stock.daily', ['section' => 'stock', 'movement_date' => $movementDate]) }}" class="list-group-item list-group-item-action {{ $section === 'stock' ? 'active' : '' }}">Stock</a>
      <a href="{{ route('employee.ui.stock.daily', ['section' => 'sortie', 'movement_date' => $movementDate]) }}" class="list-group-item list-group-item-action {{ $section === 'sortie' ? 'active' : '' }}">Sortie</a>
    </div>
  </div>
      ...
      <div class="card shadow-sm mb-3">
        <div class="card-body">
          <form method="GET" class="row g-2 align-items-end">
            ...
            <div class="col-md-4"><label class="form-label">Date</label><input type="date" class="form-control" name="movement_date" value="{{ $movementDate }}"></div>
      ...
      <div class="card shadow-sm mb-3">
        ...
        <div class="card-body">
          <form method="POST" action="{{ route('employee.ui.stock.daily.outgoing.store') }}" class="row g-2 align-items-end">
            @csrf
            <div class="col-md-4"><label class="form-label">Article</label><select class="form-select" name="stock_item_id" required><option value="">Choisir...</option>@foreach($items as $item)<option value="{{ $item->id }}">{{ $item->name }} ({{ $item->unit }})</option>@endforeach</select></div>
            ...
            <input type="hidden" name="section" value="sortie">
