      ...
      <div class="card-body">
        <form method="POST" action="{{ route('employee.ui.orders.store') }}" class="vstack gap-2">
          ...
          <div class="border rounded p-2">
            <div class="d-flex justify-content-between align-items-center mb-2"><strong>Articles</strong><button type="button" class="btn btn-sm btn-outline-secondary" id="empAddItemBtn">+ Ajouter ligne</button></div>
            <div id="empItemsContainer" class="vstack gap-2"></div>
            <div class="small text-muted mt-2">Total: <strong id="empTotalAmount">0</strong> FCFA</div>
          </div>
            ...
            @forelse($orders as $order)
              ...
              <tr>
                ...
                <td class="d-flex gap-1 flex-wrap">
                  ...
                  @endif
                  <button class="btn btn-sm btn-outline-secondary" data-bs-toggle="modal" data-bs-target="#editOrder{{ $order->id }}">Modifier</button>
                  <form method="POST" action="{{ route('employee.ui.orders.ready', $order) }}">@csrf<button class="btn btn-sm btn-outline-primary" type="submit">Prête</button></form>
                  <form method="POST" action="{{ route('employee.ui.orders.picked', $order) }}">@csrf<button class="btn btn-sm btn-outline-success" type="submit">Retirée</button></form>
                </td>
              </tr>
              @if($remaining>0)
                <tr id="pay{{ $order->id }}" class="collapse">
                  <td colspan="8">
                    <form method="POST" action="{{ route('employee.ui.orders.payments.store', $order) }}" class="row g-2">
                      @csrf
                      <div class="col-md-4"><input class="form-control" type="number" min="1" max="{{ $remaining }}" step="0.01" name="amount" placeholder="Montant" required></div>
                      <div class="col-md-4"><select class="form-select" name="payment_method"><option value="">Moyen de paiement</option><option value="cash">Cash</option><option value="wave">Wave</option><option value="orange_money">Orange Money</option><option value="card">Carte bancaire</option></select></div>
                      <div class="col-md-4 d-grid"><button class="btn btn-warning">Ajouter paiement</button></div>
                    </form>
                  </td>
                </tr>
              @endif
...
@foreach($orders as $order)
  <div class="modal fade" id="editOrder{{ $order->id }}" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog">
      <div class="modal-content">
        <div class="modal-header"><h5 class="modal-title">Modifier commande</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
