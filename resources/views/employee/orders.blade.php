...
<div class="row g-4">
    ...
    <div class="card shadow-sm">
      ...
      <div class="card-body">
        <form method="POST" action="{{ route('employee.ui.orders.store') }}" class="vstack gap-2">
          ...
          <select class="form-select" name="status">@foreach($orderStatuses as $status)<option value="{{ $status->code }}" @selected($status->code==='pending')>{{ $status->label }}</option>@endforeach</select><input class="form-control" type="number" min="0" step="0.01" name="discount_amount" placeholder="Réduction (optionnel)">
  ...
  <div class="col-lg-7">
    <div class="card"><div class="card-header">Commandes de mon agence</div><div class="table-responsive"><table class="table mb-0 datatable align-middle"><thead><tr><th>Référence</th><th>Client</th><th>Total</th><th>Payé</th><th>Reste</th><th>Statut</th><th>Actions</th></tr></thead><tbody>@forelse($orders as $order)@php $remaining=max(0,(float)$order->total-(float)$order->advance_amount); $status=$statusMap[$order->status] ?? null; @endphp<tr><td>{{ $order->reference }}</td><td>{{ $order->client?->name ?? '-' }}</td><td>{{ number_format($order->total,0,',',' ') }}</td><td>{{ number_format($order->advance_amount ?? 0,0,',',' ') }}</td><td>{{ number_format($remaining,0,',',' ') }}</td><td><span class="badge bg-{{ $status->badge_class ?? 'secondary' }}{{ ($status?->badge_class==='warning') ? ' text-dark' : '' }}">{{ $status->label ?? $order->status }}</span></td><td class="d-flex gap-1 flex-wrap">@if($order->status==='pending')<a class="btn btn-sm btn-outline-secondary" href="{{ route('employee.ui.orders.edit', $order) }}">Modifier</a>@if($remaining>0)<button class="btn btn-sm btn-outline-warning" data-bs-toggle="modal" data-bs-target="#payModal{{ $order->id }}">Paiement</button>@endif<form method="POST" action="{{ route('employee.ui.orders.ready', $order) }}">@csrf<button class="btn btn-sm btn-outline-primary">Prête</button></form>@if($remaining<=0)<form method="POST" action="{{ route('employee.ui.orders.picked', $order) }}">@csrf<button class="btn btn-sm btn-outline-success">Retirée</button></form>@endif@endif</td></tr>@empty<tr><td colspan="7" class="text-center text-muted">Aucune commande</td></tr>@endforelse</tbody></table></div></div>
  </div>
</div>
