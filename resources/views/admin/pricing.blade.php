...
@section('content')
@php($section = request()->query('section','packs'))
<div class="row g-4">
  <div class="col-lg-3">
    <div class="list-group shadow-sm">
      <a href="{{ route('admin.ui.pricing', ['section' => 'packs']) }}" class="list-group-item list-group-item-action {{ $section === 'packs' ? 'active' : '' }}">Packs</a>
      <a href="{{ route('admin.ui.pricing', ['section' => 'custom']) }}" class="list-group-item list-group-item-action {{ $section === 'custom' ? 'active' : '' }}">Demandes personnalisées</a>
    </div>
  </div>
  ...
  <div class="col-lg-9">
      ...
      <div class="row g-4">
        <div class="col-lg-5">
          <div class="card shadow-sm">
            <div class="card-header bg-white">Créer un pack standard</div>
            <div class="card-body">
    ...
    @else
      <div class="row g-4">
        ...
        <div class="col-lg-7">
          <div class="card shadow-sm">
            <div class="card-header bg-white">Demandes personnalisées et plans personnalisés</div>
            <div class="table-responsive">
              <table class="table mb-0">
                <thead><tr><th>Pressing</th><th>Besoins</th><th>Prix</th><th>Type</th></tr></thead>
                <tbody>
                @foreach($customPlans as $plan)
                  <tr><td>{{ $plan->pressing?->name ?? '-' }}</td><td class="small">Agences: {{ $plan->max_agencies }} · Employés: {{ $plan->max_employees }}</td><td>{{ number_format((float)$plan->monthly_price,0,',',' ') }} FCFA</td><td><span class="badge text-bg-primary">Plan perso</span></td></tr>
                @endforeach
                @forelse($customRequests as $req)
                  <tr>
                    <td>{{ $req->pressing?->name ?? '-' }}</td>
                    <td class="small">Agences: {{ $req->requested_agencies }} · Employés: {{ $req->requested_employees }}</td>
                    <td>{{ number_format((float)$req->estimated_price,0,',',' ') }} FCFA</td>
                    <td><span class="badge text-bg-secondary">Demande</span></td>
                  </tr>
                @empty
