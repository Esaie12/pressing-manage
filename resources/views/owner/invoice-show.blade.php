@extends('layouts.app')
@section('title','Owner - Facture')
@section('heading','Owner • Détail facture')
@section('content')
@php
    $template = $invoice->pressing?->invoice_template ?? 'classic';
    $color = $invoice->pressing?->invoice_primary_color ?? '#0d6efd';
    $welcome = $invoice->pressing?->invoice_welcome_message ?: 'Merci pour votre confiance.';
@endphp
...
<div class="card shadow-sm">
  ...
  <div class="card-body">
    @if($invoice->pressing?->invoice_logo_path)<img src="{{ asset('storage/'.$invoice->pressing->invoice_logo_path) }}" alt="Logo" style="max-height:60px" class="mb-2">@endif
...
<div class="card shadow border-0" style="border-top: 6px solid {{ $color }} !important;">
  <div class="card-body">
    <div class="d-flex justify-content-between align-items-start mb-3">
      <div>@if($invoice->pressing?->invoice_logo_path)<img src="{{ asset('storage/'.$invoice->pressing->invoice_logo_path) }}" alt="Logo" style="max-height:50px" class="mb-2">@endif<h4 class="mb-0">{{ $invoice->pressing?->name }}</h4><small class="text-muted">FACTURE MODERNE</small></div>
...
<div class="p-4 bg-white border rounded-3">
  ...
  @if($invoice->pressing?->invoice_logo_path)<img src="{{ asset('storage/'.$invoice->pressing->invoice_logo_path) }}" alt="Logo" style="max-height:45px" class="mb-2">@endif
