...
<div class="row g-4">
  <div class="col-lg-4">
    <div class="card shadow-sm">
      ...
      <div class="card-body">
        <form method="POST" action="{{ route('owner.ui.employees.store') }}" class="vstack gap-2">@csrf
  ...
  <div class="col-lg-8">
    <div class="card">
      ...
      <div class="table-responsive"><table class="table mb-0 datatable"><thead><tr><th>Nom</th><th>Email</th><th>Agence</th><th>Statut</th><th>Action</th></tr></thead><tbody>@forelse($employees as $employee)<tr><td>{{ $employee->name }}</td><td>{{ $employee->email }}</td><td>{{ $employee->agency?->name ?? '-' }}</td><td>{!! $employee->is_active ? '<span class="badge text-bg-success">Actif</span>' : '<span class="badge text-bg-secondary">Bloqué</span>' !!}</td><td class="d-flex gap-1"><form method="POST" action="{{ route('owner.ui.employees.toggle', $employee) }}">@csrf<button class="btn btn-sm btn-outline-warning" type="submit">{{ $employee->is_active ? 'Bloquer' : 'Réactiver' }}</button></form><button class="btn btn-sm btn-outline-primary" data-bs-toggle="modal" data-bs-target="#pwd{{ $employee->id }}">Nouveau mot de passe</button></td></tr>
<div class="modal fade" id="pwd{{ $employee->id }}" tabindex="-1"><div class="modal-dialog"><div class="modal-content"><div class="modal-header"><h5 class="modal-title">Réinitialiser mot de passe</h5><button class="btn-close" data-bs-dismiss="modal"></button></div><div class="modal-body"><form method="POST" action="{{ route('owner.ui.employees.password', $employee) }}" class="vstack gap-2">@csrf<input class="form-control" type="password" name="password" minlength="8" placeholder="Nouveau mot de passe" required><input class="form-control" type="password" name="password_confirmation" minlength="8" placeholder="Confirmation" required><button class="btn btn-primary">Enregistrer</button></form></div></div></div></div>
@empty<tr><td colspan="5" class="text-center text-muted">Aucun employé</td></tr>@endforelse</tbody></table></div>
    </div>
  </div>
</div>
@endsection
