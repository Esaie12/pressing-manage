          ...
          <tbody>
            @foreach($services as $service)
              <tr>
                ...
                <td>{!! $service->is_active ? '<span class="badge text-bg-success">Actif</span>' : '<span class="badge text-bg-secondary">Inactif</span>' !!}</td>
                <td class="d-flex gap-1 flex-wrap">
                  @if(!$service->deleted_at)
                    <form method="POST" action="{{ route('owner.ui.services.toggle', $service) }}">@csrf<button class="btn btn-sm btn-outline-warning">{{ $service->is_active ? 'Désactiver' : 'Activer' }}</button></form>
                    <button class="btn btn-sm btn-outline-primary" data-bs-toggle="modal" data-bs-target="#editService{{ $service->id }}">Modifier</button>
                    <form method="POST" action="{{ route('owner.ui.services.delete', $service) }}">@csrf<button class="btn btn-sm btn-outline-danger">Supprimer</button></form>
                  @else
                    <form method="POST" action="{{ route('owner.ui.services.forceDelete', $service->id) }}" onsubmit="return confirm('Suppression définitive ?')">@csrf<button class="btn btn-sm btn-danger">Supprimer définitivement</button></form>
                  @endif
                </td>
              </tr>
            @endforeach
      ...
      <div class="modal-dialog">
        <div class="modal-content">
          ...
          <div class="modal-body">
            <form method="POST" action="{{ route('owner.ui.services.update', $service) }}" class="vstack gap-2">
              @csrf
