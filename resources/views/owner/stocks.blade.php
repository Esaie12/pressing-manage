...
<div class="row g-3 mb-3">
  <div class="col-md-4"><div class="card shadow-sm"><div class="card-body"><div class="small text-muted">Nombre d'articles</div><div class="h4 mb-0">{{ $totalArticles }}</div></div></div></div>
  <div class="col-md-4"><div class="card shadow-sm"><div class="card-body"><div class="small text-muted">Articles en stock</div><div class="h4 mb-0 text-success">{{ $inStockArticles }}</div></div></div></div>
  <div class="col-md-4"><div class="card shadow-sm"><div class="card-body"><div class="small text-muted">Articles en rupture</div><div class="h4 mb-0 text-danger">{{ $outOfStockArticles }}</div></div></div></div>
</div>
    ...
    <div class="list-group shadow-sm">
      ...
      <div class="list-group-item">
        ...
        <form method="GET">
          ...
          <select class="form-select form-select-sm mb-2" name="scope">
          ...
          <button class="btn btn-sm btn-outline-primary w-100">Appliquer</button>
        </form>
      </div>
    </div>
  </div>
              ...
              <tr>
                <td>
                  <form method="POST" action="{{ route('owner.ui.stocks.items.update', $item) }}" class="row g-2 align-items-center">
                    @csrf
                    <div class="col-md-5"><input class="form-control form-control-sm" name="name" value="{{ $item->name }}" required></div>
                    <div class="col-md-3"><input class="form-control form-control-sm" name="sku" value="{{ $item->sku }}"></div>
                    <div class="col-md-2"><select class="form-select form-select-sm" name="unit" required>@foreach(['unité','kg','litre','ml','paquet','carton','mètre'] as $unit)<option value="{{ $unit }}" @selected($item->unit === $unit)>{{ $unit }}</option>@endforeach</select></div>
                    ...
                    <div class="col-12 d-flex gap-1 justify-content-end"><button class="btn btn-sm btn-outline-primary">Modifier</button></div>
                  </form>
                </td>
                <td>{{ $item->sku ?: '-' }}</td>
                <td>{{ $item->unit }}</td>
                ...
                <td class="text-end">
                  <form method="POST" action="{{ route('owner.ui.stocks.items.delete', $item) }}" onsubmit="return confirm('Supprimer cet article ?')">@csrf<button class="btn btn-sm btn-outline-danger">Supprimer</button></form>
                </td>
              </tr>
            @empty
      ...
      <div class="card shadow-sm mb-3">
        ...
        <div class="card-body">
          <form method="POST" action="{{ route('owner.ui.stocks.movements.store') }}" class="row g-2">
            @csrf
            <div class="col-md-4"><label class="form-label">Article</label><select class="form-select" name="stock_item_id" required><option value="">Choisir...</option>@foreach($activeItems as $item)<option value="{{ $item->id }}">{{ $item->name }} ({{ $item->unit }})</option>@endforeach</select></div>
            <div class="col-md-3"><label class="form-label">Type</label><select class="form-select" name="movement_type" required><option value="entree">Entrée</option><option value="sortie">Sortie</option><option value="transfert">Transfert</option><option value="perte_casse">Perte / casse</option></select></div>
            ...
            <div class="col-12"><div class="small text-muted">Entrée = Destination, Sortie/Perte = Source, Transfert = Source + Destination.</div></div>
        ...
        <div class="table-responsive">
          <table class="table mb-0 align-middle">
            ...
            @forelse($movements as $m)
              @php
                $typeClass = match($m->movement_type) {'entree' => 'text-bg-success', 'sortie' => 'text-bg-warning', 'transfert' => 'text-bg-primary', 'perte_casse' => 'text-bg-danger', default => 'text-bg-secondary'};
              ...
              <tr>
                ...
                <td>@if($m->movement_type === 'transfert'){{ $m->sourceAgency?->name ?? 'Magasin central' }} → {{ $m->targetAgency?->name ?? 'Magasin central' }}@else{{ $m->agency?->name ?? 'Magasin central' }}@endif @if($m->note)<div class="small text-muted">{{ $m->note }}</div>@endif</td>
                <td class="text-end">{{ number_format((float)$m->quantity,2,',',' ') }}</td>
                <td>{{ $m->user?->name ?? '-' }}</td>
                <td class="text-end">@if($canManage)<a href="{{ route('owner.ui.stocks.movements.edit', $m) }}" class="btn btn-sm btn-outline-primary">Modifier</a><form method="POST" action="{{ route('owner.ui.stocks.movements.delete', $m) }}" class="d-inline" onsubmit="return confirm('Supprimer ce mouvement ?')">@csrf<button class="btn btn-sm btn-outline-danger">Supprimer</button></form>@else<span class="badge text-bg-secondary">Verrouillé</span>@endif</td>
              </tr>
            @empty
        ...
        <div class="table-responsive">
          <table class="table mb-0">
            ...
            @forelse($balances as $balance)
              @php $alert = $pressing->stock_mode === 'central' ? (float)($balance->item?->alert_quantity_central ?? 0) : (float)($balance->item?->alert_quantity_agency ?? 0); $isAlert = $alert > 0 && (float)$balance->quantity <= $alert; @endphp
              <tr class="{{ $isAlert ? 'table-danger' : '' }}"><td>{{ $balance->item?->name }}</td><td>{{ $balance->agency?->name ?? 'Magasin central' }}</td><td class="text-end fw-semibold">{{ number_format((float)$balance->quantity,2,',',' ') }}</td><td>{{ $alert > 0 ? number_format($alert,2,',',' ') : '-' }}</td></tr>
            @empty
